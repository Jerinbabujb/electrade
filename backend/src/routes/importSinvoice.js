...
router.post('/', upload.single('backup'), async (req, res) => {
  ...
  try {
    ...
    if (mode === 'replace') {
      ...
      await client.query(`DELETE FROM delivery_note_items  WHERE EXISTS (SELECT 1 FROM delivery_notes d WHERE d.id=dn_id AND d.company_id=$1)`, [companyId])
      await client.query(`DELETE FROM delivery_notes       WHERE company_id=$1`, [companyId])
      await client.query(`DELETE FROM cheques              WHERE company_id=$1`, [companyId])
      await client.query(`UPDATE crm_opportunities         SET quotation_id=NULL WHERE EXISTS (SELECT 1 FROM invoices i WHERE i.id=quotation_id AND i.company_id=$1)`, [companyId])
